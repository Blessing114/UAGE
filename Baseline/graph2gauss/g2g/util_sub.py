...
def load_ppi(file_name,nodes_gt):
    input_filename = '../../test/datasets/'+file_name
    # read ppi data
    ppi = []
    ...
    nodes_raw = list(set([r[0] for r in ppi] + [r[1] for r in ppi]))
    nodes = nodes_raw.copy()
    for gn in nodes_raw:
        if gn not in nodes_gt:
            nodes.remove(gn)

    A = np.identity(len(nodes))
    for r in ppi:
        if r[0] in nodes and r[1] in nodes:
            row_index = nodes.index(r[0])
            col_index = nodes.index(r[1])
            if len(r)>2:
                A[row_index][col_index] = float(r[2]) if float(r[2])>0.55 else float(0)
            else:
...
#         for g in gt_raw:
#             gt_nodes.update(g)
#         nodes_comm = gt_nodes.copy()
#         for gn in gt_nodes:
#             if gn not in nodes:
#                 nodes_comm.remove(gn)
#         nodes_comm = list(nodes_comm)
#         # form ground truth theta
...
def my_cluster(x,gt_raw,nodes,nodes_gt):
    n_clusters = 10
    X = StandardScaler().fit_transform(x)  #标准化
    cluster=KMeans(n_clusters=n_clusters).fit(X) 
    res = cluster.labels_
    result_mat = np.identity(len(nodes_gt))
    nodes_comm = list(set(nodes) & set(nodes_gt))
    gt_comms = np.identity(len(nodes_comm))
    for k in range(len(nodes_gt)):
        node1 = nodes_gt[k]
        if node1 in nodes_comm:
            index1_target = nodes_comm.index(node1)
            for i in range(k+1,len(nodes_gt)):
                node2 = nodes_gt[i]
                if node2 in nodes_comm:
                    index2_target = nodes_comm.index(node2)
                    gt_comms[index1_target, index2_target] = gt_raw[i, k]
                    gt_comms[index2_target, index1_target] = gt_raw[k, i]
