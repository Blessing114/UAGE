...
from utils import load_dataset, load_ppi,score_link_prediction, score_node_classification, read_gt, my_cluster
import pickle
import pandas as pd
import numpy as np

fileName = '../../test/pubmed-disturbed10.npz'
#需重跑pubmed-disturbed10.npz
# fileName = '../data/cora_ml.npz'
# g = load_dataset(fileName)
# # g = load_dataset('../../test/cora_ml.npz')
# A, X, z = g['A'], g['X'], g['z']
ppiName = 'krogan2006_extended.txt'
gt_dataset = 'sgd.txt'
A, X,nodes = load_ppi(ppiName)
...
pickle.dump({'mu': mu, 'sigma':sigma },
            open('g2g_embedding_%s'%(ppiName.split('/')[-1].split('.')[0]), 'wb'))
print(mu.shape)

result=pickle.load(open('g2g_embedding_%s'%(ppiName.split('/')[-1].split('.')[0]), 'rb'))
print(len(nodes))
